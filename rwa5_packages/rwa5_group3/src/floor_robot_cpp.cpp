      ...
      planning_scene_()
{
  ...
  order_timer_cbg_ = this->create_callback_group(
  ...
  quality_checker_ = this->create_client<ariac_msgs::srv::PerformQualityCheck>(
      "/ariac/perform_quality_check", rmw_qos_profile_services_default, order_timer_cbg_);
    
  // subscription to /ariac/competition_state
  rclcpp::SubscriptionOptions competition_state_options;
  competition_timer_cbg_ = this->create_callback_group(
  ...
  agv_timer_cbg_ = this->create_callback_group(
      rclcpp::CallbackGroupType::Reentrant);
...
bool FloorRobot::end_competition()
{
  ...
  {
    RCLCPP_INFO_STREAM(get_logger(), "Waiting for AGVs to reach warehouse and Order submission...");
  }
    ...
    geometry_msgs::msg::Pose model_pose)
{
  ...
  std::string package_share_directory = ament_index_cpp::get_package_share_directory("rwa5_group3");
...
bool FloorRobot::complete_orders()
{
  ...
  while (high_priority_orders_.size() == 0 && low_priority_orders_.size() == 0)
  {
  }
  ...
  while (true)
  {
    ...
    else if (high_priority_orders_.size() == 0 && low_priority_orders_.size() == 0 && active_orders_.size() == 0 && interrupted_orders_.size() == 0 && completed_orders_.size() == 0)
    {
      if (competition_state_ != ariac_msgs::msg::CompetitionState::ORDER_ANNOUNCEMENTS_DONE && competition_state_ != ariac_msgs::msg::CompetitionState::ENDED)
      {
        // go_home();
        // wait for more orders
        RCLCPP_INFO(get_logger(), "Waiting for orders...");
        while (high_priority_orders_.size() == 0 && low_priority_orders_.size() == 0 && active_orders_.size() == 0 && interrupted_orders_.size() == 0)
        {
        }
      }
...
void FloorRobot::update_parts_vector() {
  auto part_update_request = std::make_shared<rwa5_group3::srv::AdvancedCamera::Request>();
  part_update_request->request_bins = true;
  part_update_request->request_kts = false;
  auto result_part_update = advanced_camera_client_->async_send_request(part_update_request);
  result_part_update.wait();
  if (!result_part_update.get()->response_left_bins && !result_part_update.get()->response_right_bins)
  {
...
void FloorRobot::update_kts_vector() {
  auto kts_update_request = std::make_shared<rwa5_group3::srv::AdvancedCamera::Request>();
  kts_update_request->request_bins = false;
  kts_update_request->request_kts = true;
  auto result_kts_update = advanced_camera_client_->async_send_request(kts_update_request);
  result_kts_update.wait();
  if (!result_kts_update.get()->response_kts1 && !result_kts_update.get()->response_kts2)
  {
    RCLCPP_INFO_STREAM(get_logger(), "==================================================");
    RCLCPP_INFO_STREAM(get_logger(), "Woops! PART UPDATE FAILED CHECK CODE");
    RCLCPP_INFO_STREAM(get_logger(), "==================================================");
...
bool FloorRobot::complete_kitting_task(ariac_msgs::msg::Order order)
{
  if (order.priority == 0) {
    ...
    if (tray_on_agv_[order.kitting_task.agv_number] != order.kitting_task.tray_id)
    {
      if (pick_and_place_tray(order.kitting_task.tray_id, order.kitting_task.agv_number) == true)
      {
        tray_on_agv_[order.kitting_task.agv_number] = order.kitting_task.tray_id;
        FloorRobot::update_kts_vector();
      } else {
        RCLCPP_INFO_STREAM(get_logger(), "==================================================");
        RCLCPP_INFO_STREAM(get_logger(), "Woops! Order interrupted at pick_and_place_tray after gripper change");
        RCLCPP_INFO_STREAM(get_logger(), "==================================================");
        return false;
      }
    }
    ...
    {
      RCLCPP_INFO_STREAM(get_logger(), "==================================================");
      RCLCPP_INFO_STREAM(get_logger(), "Woops! Order interrupted at pick_and_place_tray after Putting THE LOW PRIORITY TRAY");
      RCLCPP_INFO_STREAM(get_logger(), "==================================================");
      return false;
    }
    ...
    {
      if (parts_in_tray_.find(std::make_tuple(order.kitting_task.agv_number, order.kitting_task.tray_id, kit_part.quadrant)) != parts_in_tray_.end())
      {
        continue;
      }
      ...
      parts_in_tray_[std::make_tuple(order.kitting_task.agv_number, order.kitting_task.tray_id, kit_part.quadrant)] = kit_part.part;
      FloorRobot::update_parts_vector();
      if (!high_priority_orders_.empty())
      {
        RCLCPP_INFO_STREAM(get_logger(), "==================================================");
        RCLCPP_INFO_STREAM(get_logger(), "Woops! Order interrupted at pick_bin_part");
        RCLCPP_INFO_STREAM(get_logger(), "==================================================");
        return false;
      }
    }
  ...
  // For high priority orders 
    ...
    // go_home();
    pick_and_place_tray(order.kitting_task.tray_id, order.kitting_task.agv_number);
    FloorRobot::update_kts_vector();
    tray_on_agv_[order.kitting_task.agv_number] = order.kitting_task.tray_id;
    for (auto kit_part : order.kitting_task.parts)
    {
      // if (parts_in_tray_.find(std::make_pair(order.kitting_task.tray_id, kit_part.quadrant)) != parts_in_tray_.end())
      // {
      //   continue;
      // }
      parts_in_tray_[std::make_tuple(order.kitting_task.agv_number, order.kitting_task.tray_id, kit_part.quadrant)] = kit_part.part;
